...
# browser()
  ...
  #For each trip
  dat <- dat %>% group_by(drvid, trip_id, dyear) %>% mutate(trip_hpounds = sum(hpounds),
    trip_apounds = sum(apounds)) %>% group_by(drvid, trip_id, species, dyear) %>%
  mutate(trip_hpounds = unique(trip_hpounds), trip_apounds = unique(trip_apounds),
      trip_spp_hperc = sum(hpounds) / trip_hpounds, 
      trip_spp_aperc = sum(apounds) / trip_apounds) %>% as.data.frame

  #For each vessel
  dat <- dat %>% group_by(drvid, dyear) %>% mutate(vess_hpounds = sum(hpounds),
    vess_apounds = sum(apounds)) %>% group_by(drvid, species, dyear) %>%
    mutate(vess_hpounds = unique(vess_hpounds), vess_apounds = unique(vess_apounds),
      vess_spp_hperc = sum(hpounds) / vess_hpounds,
      vess_spp_aperc = sum(apounds) / vess_apounds) %>% as.data.frame
  
  #For each cluster
  dat <- dat %>% group_by(clust, dyear) %>% mutate(clust_hpounds = sum(hpounds),
    clust_apounds = sum(apounds)) %>% group_by(clust, dyear, species) %>%
    mutate(clust_hpounds = unique(clust_hpounds), clust_apounds = unique(clust_apounds),
